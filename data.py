...
def get_dataloader(path, config):
    ...
    num_workers=8
    dataset = dataset_cls(path=path, config=config, pre_transforms=pre_transforms)
...
class AnimDataset(Dataset):
    ...
    def __init__(self, transforms=None, pre_transforms=None, batch_size=1, *args, **kwargs):
            ...
            for anim_name in anims:
                ...
                for take in takes:
                    ...
                    self._anim_info.append(
                            {'character': character, 'anim_name': anim_name}
                    )

                    if character not in self._anim_structure:
                        self._anim_structure[character] = {}
                    self._anim_structure[character][anim_name] = (self.num_frames, self.num_frames+anim.shape[0])
...
class MixamoDataset(AnimDataset):
    ...
    def __getitem__(self, index):
        ...
        ind_11 = np.random.randint(self._anim_structure[c_1][m_1][0], self._anim_structure[c_1][m_1][1]+1)
        ind_12 = np.random.randint(self._anim_structure[c_1][m_2][0], self._anim_structure[c_1][m_2][1]+1)
        ind_21 = np.random.randint(self._anim_structure[c_2][m_1][0], self._anim_structure[c_2][m_1][1]+1)
        ind_22 = np.random.randint(self._anim_structure[c_2][m_2][0], self._anim_structure[c_2][m_2][1]+1)
        ...
        # Creates 2D versions
        data_111_2d = self.to_2d(data_111)
        data_121_2d = self.to_2d(data_121)
        data_112_2d = self.to_2d(data_112)
        data_122_2d = self.to_2d(data_122)
        data_211_2d = self.to_2d(data_211)
        data_221_2d = self.to_2d(data_221)
        data_212_2d = self.to_2d(data_212)
        data_222_2d = self.to_2d(data_222)

        # Final steps
        data_111 = self.final_steps(data_111)
        data_121 = self.final_steps(data_121)
        data_112 = self.final_steps(data_112)
        data_122 = self.final_steps(data_122)
        data_211 = self.final_steps(data_211)
        data_221 = self.final_steps(data_221)
        data_212 = self.final_steps(data_212)
        data_222 = self.final_steps(data_222)
...
class MixamoLimbScaleDataset(AnimDataset):
    ...
    def __getitem__(self, index):
        ...
        anim_indices = np.random.randint(anim_rngs[:,0], anim_rngs[:,0]+1)
...
if __name__ == '__main__':
    ...
    #ds = AnimDataset(pre_transforms=pre_transforms)
    ...
    for i, pose in enumerate(dl):
        ...
        continue
        #visualize_mpl(pose[0], show_basis=False, ds=ds)
